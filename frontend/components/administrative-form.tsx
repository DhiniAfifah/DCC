...
export default function AdministrativeForm({
  ...
  const onSubmit = async (data: any) => {
    ...
    if (!data.tgl_mulai || !data.tgl_akhir || !data.tgl_pengesahan) {
      alert("Tanggal tidak boleh kosong!");
      return;
    }

    // Pastikan format tanggal diubah menjadi yyyy-MM-dd jika perlu
    data.tgl_akhir = format(new Date(data.tgl_akhir), "yyyy-MM-dd");
    data.tgl_mulai = format(new Date(data.tgl_mulai), "yyyy-MM-dd");
    data.tgl_pengesahan = format(new Date(data.tgl_pengesahan), "yyyy-MM-dd");

    console.log("Data yang dikirim:", data); // Verifikasi data
    try {
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <FormControl>
                            ...
                            >
                              {field.value ? (
                                // Format tanggal menggunakan "yyyy-MM-dd" sebelum ditampilkan di button
                        ...
                        <PopoverContent className="w-auto p-0" align="start">
                          <Calendar
                            ...
                            onSelect={(date) => field.onChange(date)} // field.onChange memastikan nilai tanggal dikirim
                            disabled={(date) =>
