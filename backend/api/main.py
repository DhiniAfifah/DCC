                ...
                for image in method.image:
                    if image and image.gambar:
                        ...
                        if os.path.exists(file_path):
                            mime_type = image.mimeType if hasattr(image, 'mimeType') else ""
                            with open(file_path, "rb") as img_file:
                                image_data = img_file.read()
                                base64_str = base64.b64encode(image_data).decode('utf-8')

                                image.base64 = base64_str
                                image.gambar_url = file_path
...
async def update_dcc_status(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Failed to update DCC status: {str(e)}")
